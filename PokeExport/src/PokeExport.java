...
//				writeFile();
		...
		//		writePartySize(5);
		//		addPokemonToParty(importPokemon(listAllPokemonFiles()[0]));
		//		writePokemonToData(importPokemon(listAllPokemonFiles()[0]), getPosition(0,1));
		//		writePokemonToData(importPokemon(listAllPokemonFiles()[0]), getPosition(0,2));
		...
		//		for (int x=0; x<14; x++)
		//			System.out.println(getBoxSize(x));
		////
		//		printBytes(readPokemonFromData(getPosition(0,1)));
		//		writePokemonToData(importPokemon(listAllPokemonFiles()[1]), getPosition(0,1));
		//		writeFile();
	}
	...
	void printAllPokemonDetails() 
	{
		...
		for (int x=0; x<14; x++)
		{
			for (int y=0; y<getBoxSize(x-1); y++)
	...
	public  void printDetailedParty() 
	{
		for (int x=0; x<getPartySize(); x++)
			System.out.println(parsePkm(readPokemonFromData(x)));	
	}
	...
	/**
	 * Right now, the box parameter goes -1 = 1st box, 0 = 2nd box, and so on
	 * @param box
	 * @return
	 */
	int getBoxSize(int box)
	{
		return getUnsigned(data[16384+1104*(box+1)]);
	}

	void writeBoxSize(int size, int box)
	{
		System.out.println(data[16384+1104*(box)]);
		data[16384+1104*(box)] = (byte)deSign(size);

		// write out the extra bytes for the empty party places
		for (int x=size; x<21; x++)
			data[16384+1104*(box)+1+x] = (byte) 255;
	}

	void writePartySize(int size)
	{
		data[10341] = (byte)deSign(size);

		// write out the extra bytes for the empty party places
		for (int x=size; x<7; x++)
			data[10342+x] = (byte) 255;
	}

	/**
	 *  Adds a pokemon to the party if the party isn't full
	 * @param b
	 */
	void addPokemonToParty(byte[] b)
	{
		...
		writePartySize(newsize);

		writePokemonToData(b, getPosition(0, newsize));
	}
	...
	void addPokemonToBox(byte[] b, int box)
	{
		...
		writeBoxSize(newsize, box-1);

		writePokemonToData(b, getPosition(box, newsize));
	}

	/**
	 * Parses a PKM file as a string
	 */
	String getUnknownType(byte[] b)
	{
		return "??"; /*TODO: this*/
	}
	...
	static int getHPIV(int attack, int defense, int speed, int special)
	{
		String sum = ""+ (attack%2*1000 + defense%2*100 + speed%2*10 + special%2);
	...
	/**
	 * Returns an array of length 5of IVs in integer form
	 * @param b
	 * @return	it starts with attack, defense, speed, special and finally HP
	 */
	static int[] getIVs(byte[] b)
	{
		int[] ivs = new int[5];

		ivs[0] = getAttackIV(b[43]);
		ivs[1] =  getDefenseIV(b[43]);
		ivs[2] = getSpeedIV(b[44]);
		ivs[3] = getSpecialIV(b[44]);
		ivs[4] = getHPIV(ivs[0], ivs[1], ivs[2], ivs[3]);
	...
	{		
		...
		return ""+ivs[4]+ " HP / "+ivs[0]+" ATK / "+ivs[1]+" DEF / "+ivs[2]+" SPEED / "+ivs[3]+" SPECIAL";
	}
	...
	static int getItem(byte[] b)
	{
		return getUnsigned(b[23]);
	}

	static void writeItem(int i, byte[] b)
	{
		b[23] = (byte)deSign(i);
	}
	 ...
	 */
	public  byte[] readPokemonFromData(int index)
	{
		byte[] datap;
	 ...
	 */
	public  void writePokemonToData(byte[] b, int index)
	{
	 ...
	 */
	public  void writeOT(byte[] b, int pos)
	{
		int begin = 10349+288;
	...
	public boolean metalChecksumStuff(boolean writedata)
	{
		...
		int old_checksum1 = (getUnsigned(data[dimens[2]]));
		int old_checksum2 = (getUnsigned(data[dimens[2]+1]));
	 ...
	 */
	public  byte[] readNickName(int pos)
	{
	...
	public static byte[][] importMultiplePokemon(File[] fs) throws IOException
	{
		byte[][] bytematrix = new byte[fs.length][];

		for (int x=0; x<fs.length; x++)
			bytematrix[x] = importPokemon(fs[x]);
	 ...
	 */
	public static File[] listLocalPokemonFiles()
	{
	 ...
	 * @throws IOException
	 */
	public  byte[] loadPokemon(File f) throws IOException
	{		
		// Create byte array for data to go in
	...
	{
		//pokesav = new File("/Users/Ricky/Library/Application Support/Bannister/KiGB/Battery RAM/PokŽmon Crystal.sav");
		//		pokesav = chooseFile();
		pokesav = new File("/Users/Ricky/ISO/doperoms/GBC/PokŽmon Crystal.sav");
	 ...
	 * @param pos
	 */
	public String parseOtherSpecies(int pos)
	{
		int begin = 10349; 
